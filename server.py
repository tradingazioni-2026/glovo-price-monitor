...
def get_store_links(sb_key, city_key):
    """Scrapa la homepage della città e recupera i link ai ristoranti."""
    c = CITIES[city_key]
    city_url = f"https://glovoapp.com/it/it/{c['slug']}/"
    extract_rules = {
        "store_links": {
            "selector": "a[href*='/it/it/']",
            "output": "@href",
            "type": "list"
        }
    }
    data = sb_get(sb_key, city_url, extract_rules=extract_rules, render_js=True, wait=3000)
    links = data.get("store_links") or []
    # Filtra solo link a store (hanno formato /it/it/city/store-slug/)
    store_links = []
    seen = set()
    for link in links:
        if not link:
        ...
        # formato atteso: it it city store-slug
        if len(parts) >= 5 and parts[0] in ("it","glovoapp.com") and "glovoapp.com" in link:
            if link not in seen:
                seen.add(link)
                store_links.append(link)
    return store_links[:20]


def scrape_store(sb_key, store_url, store_name):
    """Scrapa il menu di un singolo store."""
    extract_rules = {
        "products": {
            "selector": "[data-test-id='product-row'], [class*='product'], [class*='item-card'], [class*='ProductCard']",
            "output": "text",
            "type": "list"
        },
        "prices": {
            "selector": "[class*='price'], [data-test-id*='price'], [class*='Price']",
            "output": "text",
            "type": "list"
        },
        "names": {
            "selector": "[class*='product-name'], [class*='ProductName'], [data-test-id*='product-name'], h3, h4",
            "output": "text",
            "type": "list"
        },
        "categories": {
            "selector": "[class*='section-title'], [class*='SectionTitle'], [class*='category-name'], h2",
            "output": "text",
            "type": "list"
        }
    }
    try:
        data = sb_get(sb_key, store_url, extract_rules=extract_rules, render_js=True, wait=2500)
    except Exception:
        return []

    names  = [n.strip() for n in (data.get("names") or []) if n and len(n.strip()) > 1 and len(n.strip()) < 80]
    prices_raw = [p.strip() for p in (data.get("prices") or []) if p and "€" in p or any(c.isdigit() for c in (p or ""))]
    categories = [c.strip() for c in (data.get("categories") or []) if c and len(c.strip()) > 1]
        ...
        if i < len(prices_raw):
            ...
            if m:
                try:
                    ...
                    pass
        if price and 0.10 < price < 150:
            cat = categories[0] if categories else "Menu"
            products.append({
                "name":       name,
                "store":      store_name,
                "category":   cat,
...
def scrape_city(sb_key, city_key, max_items=80):
    """Scraping completo città via ScrapingBee."""
    store_links = get_store_links(sb_key, city_key)
    if not store_links:
        raise RuntimeError("Nessun link store trovato. Verifica la API key ScrapingBee.")

    all_products = []
    for store_url in store_links:
        if len(all_products) >= max_items:
            break
        # Ricava nome store dall'URL
        parts = [p for p in store_url.rstrip("/").split("/") if p]
        store_name = parts[-1].replace("-", " ").title() if parts else "Store"
        try:
            prods = scrape_store(sb_key, store_url, store_name)
...
def compute_anomalies(products, threshold):
    ...
    medians = {cat: statistics.median(v) for cat, v in by_cat.items() if len(v) >= 2}
    result = []
    for p in products:
        med = medians.get(p["category"])
        if med and med > 0:
            drop = (med - p["price"]) / med
            p.update({"avgPrice": round(med,2), "drop": round(drop,4),
                       "saving": round(med - p["price"],2), "isAnomaly": drop >= threshold})
        else:
...
def scan():
    ...
    sb_key    = (os.environ.get("SCRAPINGBEE_KEY") or body.get("sbKey") or "").strip()
    city      = (body.get("city") or "verona").strip().lower()
    threshold = float(body.get("threshold") or 0.30)
    max_items = min(int(body.get("maxItems") or 80), 200)

    if not sb_key:
        return jsonify({"error": "API key ScrapingBee mancante."}), 400
    if city not in CITIES:
        return jsonify({"error": f"Citta non supportata: {city}"}), 400

    try:
        products = scrape_city(sb_key, city, max_items)
    except Exception as e:
        return jsonify({"error": str(e)}), 502

    if not products:
        return jsonify({"error": "Nessun prodotto trovato. Controlla la API key e riprova."}), 404
    ...
    saving    = sum(p["saving"] for p in anomalies if p["saving"] > 0)

    return jsonify({
        "city": city, "total": len(products), "anomalies": len(anomalies),
        "avgDrop": round(avg_drop, 4), "totalSaving": round(saving, 2),
        "products": products,
    })
...
if __name__ == "__main__":
    ...
    print(f"\n  Glovo Price Monitor (ScrapingBee) -> http://localhost:{port}\n")
