...
BASE_HEADERS = {
    ...
    "glovo-app-version": "7.77.0",
    "glovo-language-code": "it",
    "Origin": "https://glovoapp.com",
    "Referer": "https://glovoapp.com/",
}


def gget(url, params=None, extra=None):
    h = dict(BASE_HEADERS)
    if extra:
        h.update(extra)
...
def get_city_id(lat, lon):
    loc = {"glovo-delivery-location-latitude": str(lat), "glovo-delivery-location-longitude": str(lon)}
    data = gget("https://api.glovoapp.com/v3/addresses/geocode", params={"latitude": lat, "longitude": lon}, extra=loc)
    return data.get("cityId") or (data.get("city") or {}).get("id")


def get_stores(city_id, lat, lon, limit=25):
    loc = {"glovo-delivery-location-latitude": str(lat), "glovo-delivery-location-longitude": str(lon), "glovo-city-id": str(city_id)}
    data = gget("https://api.glovoapp.com/v3/feeds/stores", params={"cityId": city_id, "latitude": lat, "longitude": lon, "limit": limit}, extra=loc)
    if isinstance(data, list): return data[:limit]
    for key in ("stores","data","elements","results"):
        if key in data: return data[key][:limit]
    return []


def get_menu(store_id, lat, lon):
    loc = {"glovo-delivery-location-latitude": str(lat), "glovo-delivery-location-longitude": str(lon)}
    try:
        data = gget(f"https://api.glovoapp.com/v3/stores/{store_id}/sections", extra=loc)
    except Exception:
        return []
    sections = data if isinstance(data, list) else (data.get("sections") or data.get("categories") or [])
    products = []
    for sec in sections:
        cat = sec.get("name") or sec.get("title") or "Altro"
        items = sec.get("elements") or sec.get("products") or sec.get("items") or []
        for item in items:
            price_raw = item.get("price") or item.get("minPrice") or 0
            try:
                price = float(price_raw)
                if price > 500: price = price / 100  # cents to euros
            except Exception:
                continue
            if price <= 0 or price > 200: continue
            products.append({
                "name":     item.get("name") or item.get("title") or "Prodotto",
                "category": cat,
                "price":    round(price, 2),
            })
    return products
...
def scrape_city(city_key, max_items=100):
    c = CITIES[city_key]
    lat, lon = c["lat"], c["lon"]
    city_id = get_city_id(lat, lon)
    if not city_id:
        raise RuntimeError("cityId non trovato")
    stores = get_stores(city_id, lat, lon, limit=25)
    if not stores:
        raise RuntimeError("Nessuno store trovato")
    all_products = []
    for store in stores:
        if len(all_products) >= max_items: break
        sid   = store.get("id") or store.get("storeId") or store.get("businessId")
        sname = store.get("name") or store.get("storeName") or store.get("title") or "Store"
        sslug = store.get("slug") or store.get("storeSlug") or ""
        surl  = f"https://glovoapp.com/it/it/{city_key}/{sslug}/" if sslug else "https://glovoapp.com"
        if not sid: continue
        try:
            prods = get_menu(sid, lat, lon)
        except Exception:
            prods = []
        for p in prods:
            p["store"]      = sname
...
def compute_anomalies(products, threshold):
    ...
    for p in products:
        ...
        if med and med > 0:
            ...
            p.update({"avgPrice": round(med,2), "drop": round(drop,4), "saving": round(med-p["price"],2), "isAnomaly": drop >= threshold})
        else:
...
def scan():
    ...
    max_items = min(int(body.get("maxItems") or 100), 300)
    if city not in CITIES:
        return jsonify({"error": f"Citta non supportata: {city}"}), 400
    try:
        products = scrape_city(city, max_items)
